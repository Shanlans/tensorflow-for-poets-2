  ...
  with tf.name_scope(layer_name):
    with tf.name_scope('weights_inter'):
      initial_inter_value = tf.truncated_normal(
          [bottleneck_tensor_size, 1000], stddev=0.001)

      inter_weights = tf.Variable(initial_inter_value, name='inter_weights')

      variable_summaries(inter_weights)
    with tf.name_scope('biases_inter'):
      inter_biases = tf.Variable(tf.zeros([1000]), name='inter_biases')
      variable_summaries(inter_biases)
    with tf.name_scope('Wx_plus_b_inter'):
      inter_value = tf.matmul(bottleneck_input, inter_weights) + inter_biases
...
#  mul_image = tf.image.per_image_standardization(resized_image)
  ...
  with tf.Session(graph=graph) as sess:
    ...
    for i in range(FLAGS.how_many_training_steps):
      ...
      if (i % FLAGS.eval_step_interval) == 0 or is_last_step:
        ...
        print('\nEvaluation time (1-image): {:.8f}s\n'.format((end-start)/100))
